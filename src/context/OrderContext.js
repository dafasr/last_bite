...
export const OrderProvider = ({ children }) => {
  ...
  const fetchOrders = useCallback(async (pageNum = 1, isRefresh = false) => {
    ...
    try {
      const response = await apiClient.get(`/orders/seller/me?page=${pageNum}&size=10`);
  ...
  const updateOrderStatus = useCallback(async (orderId, newStatus) => {
    try {
      ...
      } else if (newStatus === "CANCELLED") { // Assuming a cancel status for MerchantHomeScreen
        await apiClient.put(`/orders/${orderId}/cancel`);
      } else if (newStatus === "ACCEPTED") { // Assuming an accept status for MerchantHomeScreen
      ...
      setOrders(prevOrders => {
        ...
        if (updatedOrders.has(orderId)) {
          updatedOrders.set(orderId, { ...updatedOrders.get(orderId), status: newStatus });
        }
        return updatedOrders;
      });
  ...
  const completeOrder = useCallback(async (orderId, verificationCode) => {
    try {
      await apiClient.put(`/orders/${orderId}/complete`, { verificationCode });
      // Update local state immediately for better UX
      setOrders(prevOrders => {
        const updatedOrders = new Map(prevOrders);
        if (updatedOrders.has(orderId)) {
          updatedOrders.set(orderId, { ...updatedOrders.get(orderId), status: "COMPLETED" });
        }
        return updatedOrders;
      });
      // Re-fetch all orders to ensure data consistency with backend
